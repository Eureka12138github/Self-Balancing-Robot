...
本项目初衷是实现平衡车的直立控制，但因蓝牙模块损坏，遥控功能受限。为提升实用性、避免小车功能单一，计划新增**循迹功能**。当前时间紧迫，需明确开发路线，优先实现高价值功能模块。

### 功能模块规划

| 功能模块 | 优先级 | 预估耗时 | 难度评级 | 重要性 | 状态 |
|----------|--------|----------|----------|--------|------|
| **卡尔曼滤波** | ⭐⭐⭐ | 3-5 天 | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⏳ 待启动 |
| **PID 自整定** | ⭐⭐⭐⭐ | 5-7 天 | ⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⏳ 待启动 |
| **低功耗休眠** | ⭐⭐ | 2-3 天 | ⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⏳ 待启动 |
...
1. **卡尔曼滤波姿态解算**
   - **目标**：替换现有互补滤波算法，提升姿态解算精度与自适应能力
   - **技术点**：状态方程建模、协方差矩阵运算、传感器噪声参数整定
   - **预期效果**：角度误差降低至±1°以内，抗干扰能力显著提升

2. **PID 参数自整定系统**
   - **目标**：实现一键自动整定 PID 参数，减少人工调试成本
   - **技术点**：继电器法 Ziegler-Nichols 整定、阶跃响应采集、性能指标评估
   - **预期效果**：非专业人员也能快速调出可用参数

3. **低功耗休眠 + 唤醒机制**
   - **目标**：优化电池供电场景下的续航能力
   - **技术点**：STM32 STOP 模式配置、EXTI 中断唤醒、时钟恢复
   - **预期效果**：待机电流从 mA 级降至 μA 级，续航提升 3-5 倍
...
| 方案 | 描述 | 优点 | 缺点 | 推荐指数 |
|------|------|------|------|----------|
| **方案 A** | 先深入钻研卡尔曼滤波数学原理，再从头编写实现代码 | 理解透彻，便于后续优化和调试 | 耗时长（需 1-2 周学习线性代数、矩阵论、最优估计理论） | ⭐⭐ |
| **方案 B** | 直接复用成熟的开源卡尔曼滤波代码库，优先跑通功能，后续再优化理解 | 快速出成果（1-2 天即可集成），适合时间紧迫场景 | 初期调试困难时难以定位问题，知其然不知其所以然 | ⭐⭐⭐⭐⭐ |

**推荐建议**：**方案 B** 更适合当前项目进度

**理由**：
1. **时间约束**：当前时间非常紧张，方案 A 需要大量数学基础积累，不适合快速迭代
2. **平台适配**：STM32F103C8T6 资源有限（64KB Flash、20KB RAM），开源库通常已有针对嵌入式的优化版本
3. **实用主义**：项目核心目标是实现循迹功能，姿态解算只是子模块，应优先保证整体功能完成
4. **渐进式学习**：可先复用代码跑通功能，再在实际应用中逐步理解算法原理，理论与实践结合更高效

**推荐开源库**：
- [SimpleKalmanFilter](https://github.com/denisvalverde5/SimpleKalmanFilter) - 轻量级 C++ 实现，易于移植
- [Arduino-KalmanFilter](https://github.com/TKJElectronics/KalmanFilter) - 专为 MPU6050 优化，可直接参考
